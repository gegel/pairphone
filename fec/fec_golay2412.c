                                  ...
                                  unsigned int   _n)
{
    ...
    for (i=0; i<_n; i++) {
        ...
        // exploit the fact that vectors are at most 24 bits long;
        c = 0;
        p = _A[i] & _v;
...
unsigned int fec_golay2412_decode_symbol(unsigned int _sym_enc)
{
    ...
    if (ws <= 3) {
        ...
        if (s_index >= 0) {
            ...
            // step 4: compute s*P
            sP = golay2412_matrix_mul(s, golay2412_P, 12);

            // compute weight of sP (12 bits)
            wsP = liquid_count_ones_uint16(sP);
